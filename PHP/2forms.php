<!DOCTYPE html> <!-- Two forms to send to input email -->
<html lang="en">
<head>
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-4 offset-md-4 f">
        ...
        <form action="2forms.php" method="POST" autocomplete="off">
    ...
    <div class="row mb-5" style="margin-top: 10vh;">
      <div class="col-md-4 offset-md-4 f">
        <h2 class="text-center">Message 2</h2>

        <?php
          if(isset($_POST['send2'])){
            $recipient2 = $_POST['email2'];
            $subject2 = $_POST['subject2'];
            $message2 = $_POST['message2'];
            $sender2 = "from: syed.zain4401@gmail.com";

            if(empty($recipient2) || empty($subject2) || empty($message2)){
              ?>
              <div class="alert alert-danger text-center">
                <?php echo("All inputs are required")?></div>
              <?php
            }else{
              if(mail($recipient2, $subject2, $message2, $sender2)){
                ?>
                <div class="alert alert-success text-center">
                <?php echo("Mail successfully sent to $recipient2!")?>
                </div>
              <?php
                $recipient2 = "";
              }else{
        ...
        <form action="2forms.php" method="POST" autocomplete="off">
          <div class="form-group">
            <input type="email" name="email2" class="form-control" placeholder="Recipient">
          </div>
          <div class="form-group">
            <input type="text" name="subject2" class="form-control" placeholder="Subject">
          </div>
          <div class="form-group">
            <textarea name="message2" cols="30" rows="10" class="form-control" placeholder="Compose message..."></textarea>
          </div>
          <div class="form-group">
            <input type="submit" class="form-control btn btn-primary" value="Send" name="send2">
          </div>
        </form>
      </div>
    </div>
